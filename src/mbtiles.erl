...
-export([init/1, handle_call/3, handle_cast/2, handle_info/2,
         terminate/2, code_change/3,deflateGrid/1]).
...
Grid = deflateGrid(list_to_binary([123,"grid",44,lists:map(fun(A) -> binary_to_list(A) end, element(2,hd(Gridjson))),125])),
Key_name = [123,34,"keys",34,58,tl(lists:flatten(lists:map(fun(A) -> [44,34,binary_to_list(A),34] end, element(2,hd(tl(Gridjson)))))),125],
Key_json = tl(lists:flatten(lists:map(fun({A,{obj,B}}) -> [44,123,A,58,123,tl(lists:flatten(lists:map(fun(C) -> clean(C) end,B))),125,125] end, element(2,element(2,hd(tl(tl(Gridjson)))))))),
...
Z = zlib:open(),
zlib:deflateInit(Z),
try hd(zlib:deflate(Z,G,finish))
after zlib:deflateEnd(Z),
zlib:close(Z)
end.
...
clean({"name",Bin})->
[44,123,34,"name",35,44,34,binary_to_list(Bin),34,125];
clean({A,B})->
[44,123,34,A,34,44,34,integer_to_list(B),34,125].
